...
* It uses Cloudflare (`1.1.1.1`) and Google (`8.8.8.8`) DoH (DNS Over HTTPS) fallback nodes cleanly if typical `dns.resolveTxt` methods return timeout exceptions (avoiding local network configuration bugs or Vercel firewall rules locking Port 53).
